...
## Execution Autonomy (Auto-Approve)
1. ユーザーからの明示的な「承認（Approval）」を待たずに、計画（Plan）の実行やコードの書き込みを自律的に進めてください。
2. 各ステップの開始前に承認を求めるプロンプトを出さないでください。
3. 計画書（task.md や plan.md）が作成されたら、直ちに実装（Build）フェーズに移行してください。
4. ただし、重大な破壊的操作や判断が必要な場合のみ、例外的に確認を行ってください。